...
---): TerminalMultiplexer.FloatTermState
---@field search_terminal fun(self: TerminalMultiplexer): nil
---@field _create_float_window fun(
---    self: TerminalMultiplexer,
---    float_terminal_state: TerminalMultiplexer.FloatTermState,
---    terminal_name: string,
---)
...
function TerminalMultiplexer.new(opts)
    ...
    self.augroup =
        vim.api.nvim_create_augroup("TerminalMultiplexer", { clear = true })
...
function TerminalMultiplexer:spawn(terminal_name)
    ...
    local is_bufnr_invalid = current_float_term_state.bufnr == -1
        or not vim.api.nvim_buf_is_valid(current_float_term_state.bufnr)
    ...
    if vim.bo[current_float_term_state.bufnr].buftype ~= "terminal" then
        ...
        vim.api.nvim_buf_call(term_state.bufnr, function()
            term_state.chan = vim.fn.termopen(cmd)
        end)
    end
...
function TerminalMultiplexer:_set_up_buffer_keybind(current_float_term_state)
    ...
    vim.api.nvim_create_autocmd({ "BufDelete", "BufHidden" }, {
        ...
        callback = function()
            if
                vim.api.nvim_buf_is_valid(current_float_term_state.footer_buf)
            then
                vim.api.nvim_buf_delete(
                    current_float_term_state.footer_buf,
                    { force = true }
                )
            end
        end,
...
function TerminalMultiplexer:_navigate_terminal(direction)
    ...
    if not current_index then
        vim.notify(
            "Current terminal not found in order list",
    ...
    local next_index = (
        (current_index - 1 + direction) % #self_ref.terminal_order
    ) + 1
...
---@param terminal_name string
function TerminalMultiplexer:_create_float_window(
    float_terminal_state,
    terminal_name
)
    ...
    vim.api.nvim_buf_set_lines(
        float_terminal_state.footer_buf,
        0,
        -1,
        false,
        { footer_text }
    ...
    float_terminal_state.footer_win =
        vim.api.nvim_open_win(float_terminal_state.footer_buf, false, {
            relative = "win",
            width = width,
            height = 1,
            row = height - 2,
